...
namespace Birikim.Extensions
{
    ...
    public static class StringExtensions
    {
        ...
        /// <summary>
        /// Check whether a string is an valid e-mail address
        /// </summary>
        public static bool IsEmail(this string s)
        {
            var result = EmailValidator.Validate(s);
            //check for temp email services
            for (int i = 0; i < Constants.TemporaryEmailServices.ToArray().Count(); i++)
            {
                if (s.Contains(Constants.TemporaryEmailServices[i]))
                    result = false;
            }
            return result;
        }
